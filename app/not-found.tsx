...
export default function NotFound() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 flex flex-col items-center justify-center px-6 relative overflow-hidden selection:bg-zinc-900 selection:text-white">
      ...
      {/* Animated Background Elements - Pure CSS Animation */}
      ...
      <main className="max-w-2xl w-full text-center z-10 flex flex-col items-center">
        ...
        <div className="mb-8 p-6 bg-zinc-50 rounded-3xl border border-zinc-100 shadow-sm animate-slide-up" style={{ animationDelay: '0ms' }}>
          <SearchX className="w-12 h-12 text-zinc-300" strokeWidth={1.5} />
        </div>

        {/* Typography */}
        <h1 className="text-6xl md:text-8xl font-light tracking-tighter text-zinc-900 mb-4 animate-slide-up" style={{ animationDelay: '100ms' }}>
          404
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto animate-slide-up" style={{ animationDelay: '300ms' }}>
          ...
          <Link 
            href="/"
            className="group flex items-center justify-center gap-2 px-8 py-4 bg-zinc-900 text-white rounded-xl hover:bg-zinc-800 hover:shadow-lg hover:shadow-zinc-900/20 transition-all duration-300 w-full sm:w-auto focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-900 focus-visible:ring-offset-2"
          >
      ...
      {/* Footer Credit / Navigation Hint */}
      <footer className="absolute bottom-8 text-xs text-zinc-400 uppercase tracking-widest animate-fade-in" style={{ animationDelay: '500ms' }}>
        OramaCreativ &copy; {new Date().getFullYear()}
      </footer>
    </div>
  );
}